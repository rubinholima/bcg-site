...
7. **Diagnóstico no servidor** — Para isolar origem do 302: `curl -I -H "Host: www.bostoncitygroup.biz" -H "X-Forwarded-Proto: https" http://127.0.0.1:3000/dashboard` (bypass Nginx). Se 302 → Next.js; se 200 → Nginx/CloudFront. Verificar header `X-Middleware-Auth` na resposta (confirma versão do middleware).
