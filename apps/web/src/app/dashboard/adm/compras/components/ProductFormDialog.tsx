...
export function ProductFormDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const priceStr = initialUnitPrice.replace(",", ".").trim();
    const price = priceStr ? parseFloat(priceStr) : undefined;

    if (!edit && qty > 0 && (price == null || !Number.isFinite(price) || price < 0)) {
      setFeedback({
        open: true,
        title: "Preço obrigatório",
        message: "Informe o preço de entrada quando cadastrar quantidade inicial no estoque.",
      });
      return;
    }
    ...
    try {
      ...
      if (edit) {
        ...
        await api.post("/compras/products", {
          ...
          ...(qty > 0 ? { initialQuantity: qty, initialUnitPrice: price } : {}),
        });
      }
      onSuccess();
      onOpenChange(false);
      ...
      setFeedback({
        ...
        message: err instanceof Error ? err.message : "Não foi possível salvar o produto.",
      });
    } finally {
      setSaving(false);
    }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto text-foreground">
          <form onSubmit={handleSubmit}>
            <DialogHeader>
              <DialogTitle>{edit ? "Editar produto" : "Cadastrar produto"}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="product-kind">Categoria *</Label>
                <select
                  id="product-kind"
                  required
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground"
              ...
              {tenantCategories.length > 0 ? (
                <div className="grid gap-2">
                  <Label>Categorias de futebol (opcional)</Label>
              ...
              {edit ? (
                <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-2 text-sm">
                  <p className="font-medium text-foreground">Preços (atualizados nas entradas)</p>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <p className="text-xs text-muted-foreground">Última compra</p>
                      <p className="font-medium tabular-nums">{formatProductPrice(edit.purchasePrice)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Preço atual</p>
                      <p className="font-medium tabular-nums">{formatProductPrice(edit.currentPrice)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Preço médio</p>
                      <p className="font-medium tabular-nums">{formatProductPrice(edit.averagePrice)}</p>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Saldo: {edit.currentStock} {edit.unit} — use Movimentar no estoque para entrada/saída.
                  </p>
                </div>
              ) : (
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3 space-y-3">
                  <p className="text-sm font-medium text-foreground">Entrada inicial no estoque</p>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="product-initial-qty">Quantidade</Label>
                      <Input
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="product-initial-price">Preço unitário (R$)</Label>
                      <Input
                  ...
                  <p className="text-xs text-muted-foreground">
                    Se informar quantidade, o preço de entrada é obrigatório — alimenta estoque e preço médio.
                  </p>
                </div>
              )}
            </div>
