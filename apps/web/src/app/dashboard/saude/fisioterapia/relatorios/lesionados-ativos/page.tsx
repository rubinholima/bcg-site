...
export default function FisioterapiaLesionadosReportPage() {
  ...
  const [rows, setRows] = useState<PhysioReportsDashboard["activeInjured"]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get<PhysioReportsDashboard>(`/fisioterapia/reports/dashboard?${params}`);
      setRows(data?.activeInjured ?? []);
    } catch {
      setRows([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between print:hidden">
        <div>
          <Link href="/dashboard/saude/fisioterapia/relatorios" className="mb-2 inline-block text-sm text-muted-foreground hover:text-foreground">
            ← Relatórios
          </Link>
        ...
        <Button type="button" variant="outline" className="min-h-[44px]" onClick={() => window.print()}>
