...
export default function DeleteMedicoEquipePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data } = await api.get<{ name: string }>(`/medical-staff/${id}`);
        setName(data?.name ?? "");
      } catch {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Excluir profissional</h1>
          <p className="text-muted-foreground mt-1">
            Confirme a exclusão do cadastro do profissional.
          </p>
        </div>
      </div>
