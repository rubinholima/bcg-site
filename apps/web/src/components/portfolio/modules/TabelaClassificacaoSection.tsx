...
export function TabelaClassificacaoSection({
  ...
  useEffect(() => {
    if (!slug?.trim()) return;
    let cancelled = false;
    fetchFixtures(slug.trim(), fixturesContext, { forStandings: true }).then((list) => {
            ...
            <>
              ...
              >
                <table className={`w-full border-collapse text-left ${is3Col ? "min-w-0 table-fixed text-xs" : inSection ? "min-w-0 table-fixed" : "min-w-[800px]"} ${!is3Col && (inSection && (density === "minimal" || density === "compact") ? "text-xs" : "text-sm")}`}>
                  <thead className="[&_th]:sticky [&_th]:top-0 [&_th]:z-10 [&_th]:bg-zinc-900 [&_th]:shadow-[0_1px_0_0_rgba(255,255,255,0.1)]">
