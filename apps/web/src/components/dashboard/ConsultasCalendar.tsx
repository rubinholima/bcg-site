...
type ConsultationListView = "scheduled" | "completed";

const LIST_VIEW_TABS: { id: ConsultationListView; label: string }[] = [
...
export function ConsultasCalendar({
  ...
  const [listView, setListView] = useState<ConsultationListView>("scheduled");
  ...
  const scheduledConsultations = consultations.filter(isScheduledConsultation);
  const completedConsultations = consultations.filter(isCompletedConsultation);

  const applyCommonFilters = (list: Consultation[]) =>
    (dateFilter ? list.filter((c) => c.date === dateFilter) : list)
      .filter(
        (c) =>
          !nameFilter?.trim() ||
  ...
  const filteredScheduled = applyCommonFilters(scheduledConsultations);
  const filteredCompleted = applyCommonFilters(completedConsultations);
  const filtered = listView === "scheduled" ? filteredScheduled : filteredCompleted;
  ...
  const sortedDates = Object.keys(byDate).sort((a, b) =>
    listView === "completed" ? b.localeCompare(a) : a.localeCompare(b)
  );

  const calendarPool = listView === "scheduled" ? scheduledConsultations : completedConsultations;

  const dayConsultations = dateFilter
    ? calendarPool.filter((c) => c.date === dateFilter && c.time)
    : [];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 shrink-0">
        <div className="min-w-0 space-y-3">
          <div>
            ...
            <CardDescription>
              {listView === "scheduled"
                ? "Consultas pendentes — o que ainda será atendido."
                : "Consultas já realizadas — pós-atendimento e notas."}
            </CardDescription>
          </div>
          <div className="inline-flex rounded-xl border border-border/80 bg-muted/40 p-1">
            {LIST_VIEW_TABS.map((tab) => {
              const count =
                tab.id === "scheduled"
                  ? applyCommonFilters(scheduledConsultations).length
                  : applyCommonFilters(completedConsultations).length;
              return (
                <button
                  key={tab.id}
                  type="button"
                  ...
                  className={cn(
                    ...
                    listView === tab.id
                      ? tab.id === "completed"
                        ? "bg-emerald-500/15 text-emerald-800 shadow-sm dark:text-emerald-300"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(280px,340px)_1fr] lg:items-start">
        ...
        <div className="rounded-2xl border border-border/80 bg-card/40 p-4 sm:p-5 lg:sticky lg:top-4 lg:max-h-[min(78vh,820px)]">
          ...
          <div className="grid grid-cols-7 gap-1.5 text-center text-xs">
            ...
            {days.map(({ date, day }) => {
              const hasConsultation = calendarPool.some((c) => c.date === date);
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex min-h-[40px] items-center justify-center rounded-lg text-sm font-medium transition-colors",
                    isSelected
                      ? listView === "completed"
          ...
          {dateFilter ? (
            <div className="mt-5 border-t border-border/60 pt-4">
              <p className="mb-3 text-xs font-medium text-muted-foreground">
                ...
                {listView === "completed"
                  ? "horários com consultas realizadas em verde"
              ...
              <p className="text-sm text-muted-foreground">
                {consultations.length === 0
                  ...
                  : nameFilter?.trim()
                    ...
                    : listView === "scheduled"
                      ? dateFilter
                        ? "Nenhuma consulta agendada nesta data."
                        : "Nenhuma consulta agendada no momento."
                      : dateFilter
                        ? "Nenhuma consulta realizada nesta data."
                        : "Nenhuma consulta realizada ainda. Marque como realizada após o atendimento."}
              </p>
                    ...
                    {byDate[dateKey].map((c) => {
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {listView === "scheduled" && c.status !== "cancelled" && c.status !== "completed" && (
                            <>
                              <Button
                                variant="outline"
                                size="sm"
                            ...
                            <>
                              ...
                              {c.playerId ? (
                                <Button variant="outline" size="sm" asChild className="min-h-[36px]">
                                  <Link
                                    href={playerPsychologyProfileHref(c.playerId, "consultas")}
                                    title="Abrir ficha psicológica para pós-atendimento"
                                  >
