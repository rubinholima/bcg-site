...
import type { PassageirosReportDto, PrintPageSize } from "@/lib/futebol-relatorios.types";
import {
...
export function FutebolRelatorioPassageirosForm() {
  ...
  const [reportData, setReportData] = useState<PassageirosReportDto | null>(null);
  ...
  const fetchReport = async (): Promise<PassageirosReportDto | null> => {
    if (!travelId) {
      setFeedback({
        ...
        message: "Selecione a viagem para gerar a relação de passageiros.",
        variant: "warning",
      });
      return null;
    }
    try {
      const { data } = await api.get<PassageirosReportDto>(
        `/futebol-relatorios/passageiros?travelId=${encodeURIComponent(travelId)}`,
      );
      return data;
    } catch {
      setFeedback({
  ...
  const handlePreview = async () => {
    ...
    if (data) {
      setReportData(data);
      setPreviewHtml(buildPassageirosPrintHtml(data, pageSize));
      setPreviewOpen(true);
    }
    setBusy(false);
  };
  ...
  return (
    <>
      ...
      <PrintPreviewDialog
        ...
        title="Pré-visualização — Passageiros"
        html={previewHtml}
        onPrint={() => {
          if (reportData) printPassageirosReport(reportData, pageSize);
        }}
      />
