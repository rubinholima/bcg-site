...
import {
  createExpenseLineId,
  type LogisticsExpenseKind,
  type LogisticsExpenseLine,
} from "@/lib/logistica-travel-cadastros.types";
import { LogisticaComprovantePicker } from "./LogisticaComprovantePicker";
...
const SECTIONS: { kind: LogisticsExpenseKind; title: string }[] = [
  { kind: "previsto", title: "Custos previstos" },
  { kind: "adicional", title: "Custos adicionais" },
];
...
export function LogisticaExpenseLinesFields({
  ...
  const addLine = (kind: LogisticsExpenseKind) => {
    onChange([...lines, emptyLine(kind, defaultPaymentTypeId, defaultSupplierId)]);
  };
  ...
  const renderSection = (kind: LogisticsExpenseKind, title: string) => {
    const sectionLines = lines.filter((l) => (l.kind ?? "previsto") === kind);
          ...
          <div className="space-y-3">
            {sectionLines.map((line) => (
              ...
              >
                ...
                <LogisticaComprovantePicker
                  value={line.receiptUrl ?? ""}
                  onChange={(url) => updateLine(line.id, { receiptUrl: url || null })}
                  disabled={disabled}
                />
              </div>
            ))}
  ...
  return (
    <div className="space-y-6">
      {SECTIONS.map(({ kind, title }) => renderSection(kind, title))}
    </div>
  );
}
